    ...
    displayStepProgress = (stepIndex, before) => {
        ...
        this.workerPairs.forEach((pair) => {
            pair.forEach((worker, idx) => {
                if(idx){
                    below.push([worker.items.join(''),  worker.assembling ? 'wait:' + worker.timeLeftToAssemble: ''].join(' ').padEnd(10))
                } else { 
                above.push([worker.items.join(''), worker.assembling ? 'wait:' + worker.timeLeftToAssemble: ''].join(' ').padEnd(10))
                }
            })
        })
